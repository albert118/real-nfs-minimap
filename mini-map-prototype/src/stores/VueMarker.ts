...
export class VueMarker implements VueMarkerType {
  ...
  render(el: HTMLElement, appContext: AppContext) {
    ...
    // TODO: typing for renderComponent is fucky when props exist
    // @ts-ignore
    this.__component = renderComponent(markerEl, MapMarker, this.name, appContext);
    if (!this.__component) throw new Error('Failed to render the MapMarker');

    // TODO: can this be skipped now with the updated icon + marker settings?
